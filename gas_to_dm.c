...
Date: 11/12/15
Purpose: Switches gas particles from dspec_2 back to
         dm particles for use in tspec.
Notes:
************************************************/
#include <stdlib.h>
#include <stdio.h>
...
int main(int argc, char **argv)
{
   ...
   if(argc != 4)
   {
      ...
      printf("e.g. snapshot_001, proper snapshot number (i.e. 002) and file num (i.e. the number after the .)\n");
      exit(EXIT_FAILURE);
   }
...
PARTICLE_DATA *read_snapshot(char* fullname, int NumPart, int ntot_withmasses, IO_HEADER header1)
{
   ...
   for(k = 0, pc_new = pc; k < 6; k++)
   {
      for(n = 0; n < header1.npart[k]; n++)
      {
         my_fread(&P[pc_new].Rho, sizeof(float), 1, fd);
         pc_new++;
      }
   }
   SKIP2;
   ...
   for(k = 0, pc_new = pc; k < 6; k++)
   {
      for(n = 0; n < header1.npart[k]; n++)
      {
         my_fread(&P[pc_new].hsml, sizeof(float), 1, fd);
         pc_new++;
      }
   }
   SKIP2;
...
/***********************
        fake_gas
***********************/
void gas_to_dm(char *proper_snapnum, char * proper_filenum)
{
   // Changes gas particles to dm particles
   ...
   char newname[200];
	...
	/*Open file*/
   sprintf(newname, "snapshot_%s-dm.%s", proper_snapnum, proper_filenum);
   ...
   m_header.npart[1] = m_header.npart[0];
   m_header.npartTotal[1] = m_header.npartTotal[0];
   m_header.npart[0] = 0;
   m_header.npartTotal[0] = 0;
   m_header.mass[1] = m_header.mass[0];
   m_header.mass[0] = 0.0;
   ...
   // Densities
   blksize = m_header.npart[1] * sizeof(float);
   SKIP3;
   for(n = 0; n < m_header.npart[1]; n++)
   {
      my_fwrite(&M_P[pc_new].Rho, sizeof(float), 1, fd);
   }
   SKIP3;
   ...
   for(n = 0; n < m_header.npart[1]; n++)
   {
      my_fwrite(&M_P[pc_new].hsml, sizeof(float), 1, fd);
   }
   SKIP3;
